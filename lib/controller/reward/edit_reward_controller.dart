...
class EditRewardController extends GetxController {
  ...
  updateInstoreReward() async {
    ...
    ApiResponse response = await apiService.multipartRequest(
      method: "PATCH",
      endPoint: ApiEndpoints.updateReward(rewardId: model!.id),
      isAuthRequired: true,
      fields: payLoad,
  ...
  updateOnlineReward() async {
    ...
    Uri uri = Uri.parse(
      ApiEndpoints.baseUrl + ApiEndpoints.updateReward(rewardId: model!.id),
    );
  ...
  deleteRewardImage() async {
    ...
    await apiService.networkRequest(
      method: "DELETE",
      isAuthRequired: true,
      endPoint: ApiEndpoints.deleteRewardImage + model!.id,
    );
  }
}
